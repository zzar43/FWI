...
# There should only be one source. Receiver can be multiple, but suggest test start from 1.
...
# Make data
ind_source = 1;
@time true_wavefield, received_data = wave_solver_2d_pml(vel_true,Nx,Ny,h,Nt,dt,pml_len,pml_alpha,source_coor[ind_source,:]',source_vec[ind_source,:]',receiver_coor);
...
# adjoint_source = flipdim(received_data_forward, 2);
