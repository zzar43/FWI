...
function helmholtz_pml(c,Nx,Ny,omega,h,source_coor,source_func,receiver_coor,pml_len,pml_alpha)
    Nx0 = Nx + 2pml_len;
    Ny0 = Ny + 2pml_len;
    # pml_alpha = 2 * omega.^2 / (4*pi^2*20*maximum(abs.(vel_true))) * (100*h);
    pml_alpha = pml_alpha * omega.^2 / (4*pi^2*20*maximum(abs.(vel_true))) * (100*h);
    pml_value = linspace(0,pml_alpha,pml_len);

    # Extend velocity
    c_ex = zeros(Complex128,Nx0,Ny0);
    for i in 1:pml_len
    ...
    # PML Coef
    sigma_x = zeros(Complex128,Nx0,Ny0);
    sigma_y = zeros(Complex128,Nx0,Ny0);
    for i = 1:pml_len
    ...
    # Source Term
    source = zeros(Complex128,Nx0-2,Ny0-2);
    source_coor += pml_len;
    ...
    return u, received_data;
end 

function helmholtz_pml1(c,Nx,Ny,omega,h,source_coor,source_func,receiver_coor,pml_len,pml_alpha)
    Nx0 = Nx + 2pml_len;
    Ny0 = Ny + 2pml_len;
    # pml_alpha = 2 * omega.^2 / (4*pi^2*20*maximum(abs.(vel_true))) * (100*h);
    pml_alpha = pml_alpha * omega.^2 / (4*pi^2*20*maximum(abs.(vel_true))) * (100*h);
    ...
    # Source Term
    source = zeros(Complex128,Nx0-2,Ny0-2);
    source[pml_len:pml_len-1+Nx,pml_len:pml_len-1+Ny] = source_func;
    source_vec = reshape(source, (Nx0-2)*(Ny0-2));
    ...
    received_data = zeros(Complex128,Nx,Ny);
    received_data[receiver_coor[:,1],receiver_coor[:,2]] = u[receiver_coor[:,1],receiver_coor[:,2]];
    # received_data = u[receiver_coor[:,1]+(receiver_coor[:,2]-1)*Nx];
    return u, received_data;
end 
...
function make_source_vec(source_coor, source_func, h, Nx, Ny, pml_len)
    source_coor += pml_len;
...
function helmholtz_pml_simplified(c,Nx,Ny,h,omega,source_coor,source_func,pml_len,pml_alpha)
    Nx0 = Nx + 2pml_len;
    Ny0 = Ny + 2pml_len;
    pml_value = linspace(0,pml_alpha,pml_len);
    ...
    source_vec = make_source_vec(source_coor, source_func, h, Nx0, Ny0, pml_len);
