...
function wave_solver_2d_pml(c,Nx,Ny,h,Nt,dt,pml_len,pml_alpha,source_coor,source_func,receiver_coor)
 ...
 #       source[source_coor_ex[i,1], source_coor_ex[i,2], :] = (1/dt)^2*source_func[i,:];
        source[source_coor_ex[i,1], source_coor_ex[i,2], :] = source_func[i,:];
    end
...
function wave_solver_2d_pml1(c,Nx,Ny,h,Nt,dt,pml_len,pml_alpha,source_coor,source_func,receiver_coor,wavefield_record=true)
    ...
    receiver_num = size(receiver_coor,1);
    received_data = zeros(receiver_num,Nt);
    receiver_coor = receiver_coor + pml_len
    receiver_ind = (Nx+2pml_len).*(receiver_coor[:,2]-1) + receiver_coor[:,1];
    ...
    source_coor_ind = (Nx+2pml_len).*(source_coor_ex[:,2]-1) + source_coor_ex[:,1]
    # ====================
    # Main loop
    # ====================
    for iter_t = 1:Nt
        ...
        u2[source_coor_ind] += dt^2*source_func[:,iter_t];
        ...
        # record time domain wavefield
        received_data[:,iter_t] = u2[receiver_ind];
        if wavefield_record == true
            snaps_u[:,:,iter_t] = u2[pml_len+1:pml_len+Nx,pml_len+1:pml_len+Ny];
        end
    end
